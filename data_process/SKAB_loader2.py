...
class ADDataset(Dataset):
    ...
    def __getitem__(self, index):
        end = index + self.pred_len

        return self.data[end-1], self.target[end-1]
...
def data_read(args, dirs):
    ...
    ad_valid_data = df.iloc[train_len:train_len+valid_len, :-1]
    ad_test_data = df.iloc[train_len+valid_len:, :-1]
    lstfad_train_data = ad_train_data
    ...
    AD_threshold_dataset = ADDataset(ad_valid_data, scaler, args.pred_len)
    AD_test_dataset = ADDataset(ad_test_data, scaler, args.pred_len)
